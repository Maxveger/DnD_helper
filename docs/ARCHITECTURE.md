...
Срез исходников на 2026-09-20 после перехода к разговорному режиссёру. Продуктовые границы задаёт [основа проекта](PROJECT_BRIEF.md), причины смены подхода — [журнал решений](ENGINE_DESIGN_LOG.md), пользовательский процесс — [руководство ведущего](GM_ASSISTANT.md).
...
| Контент | `dnd-world@1`, `intent-simple` | Встроенное приключение или `dnd-adventure@1` |
...
flowchart TD
    ...
    F --> C[CodexProvider: независимый запрос хода]
...
CLI запускается из нейтрального временного каталога с отключёнными инструментами и пользовательской конфигурацией; обнаруженное инструментальное событие отвергается. Транскрипт короткой игровой сессии сохраняет сам Codex локально, чтобы `resume` мог продолжить разговор. Это меры адаптера, не доказательство полной изоляции произвольной версии CLI. [Контракт провайдера](CODEX_PROVIDER.md).
