...
Для следующего этапа выбран [Codex CLI как основной LLM-провайдер](CODEX_PROVIDER.md), Responses API — альтернативный. Ниже описан API-адаптер прежних историй. Новый текстовый срез использует `codex_provider.py` (штатный CLI), `free_world.py` (типы предложений, операции и подтверждение) и отдельную SQLite-базу `free-world.sqlite3`. Экран `/world` обслуживают отдельные HTTP-маршруты и статические файлы; это позволяет сохранить старые приключения. [Фактический объём и проверки](FREE_WORLD_CHECK.md). API-провайдер нового контракта ещё предстоит подключить.
