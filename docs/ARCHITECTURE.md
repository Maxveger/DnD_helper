...
Срез исходников на 2026-09-22. Активный эксперимент описан в [игровой IDE](GAME_STUDIO.md); `/world` ниже сохранён как прежний помощник. Продуктовые границы задаёт [основа проекта](PROJECT_BRIEF.md).
...
| Модель | Строгая игровая сессия + независимая сессия наблюдателя | Независимые вызовы `CodexProvider` | `OpenAIProvider`, Telegram |
...
flowchart TD
    ...
    G --> C[CodexProvider: игровая CLI-сессия]
    G --> Q[CodexProvider: отдельная сессия наблюдателя]
    F --> C
    C -->|StudioTurn| G
    Q -->|DirectorPulse| G
...
| [studio.py](../dnd_helper/studio.py) | Живая игровая беседа, отдельный наблюдатель, три канала, карточка, капсула, подтверждение и локальный бросок |
...
CLI запускается из нейтрального временного каталога с отключёнными инструментами; обнаруженное инструментальное событие отвергается. `/studio` держит один процесс App Server и продолжает точный тред через `turn/start`; `/world` остаётся эфемерным `codex exec`. Модель предлагает только дельту капсулы, а полное состояние вычисляется локально. Это меры адаптера, не доказательство полной изоляции произвольной версии CLI. [Контракт провайдера](CODEX_PROVIDER.md).
