...
Текущая версия из исходников: текстовый экран `/world`, собственный вход ведущего через Codex CLI, модель Luna. Каждая карточка создаётся независимым запросом из компактной папки текущего хода: видимые сущности, известные факты, нужные скрытые ограничения и уже вычисленные механические варианты. Приложение переводит крупные смысловые последствия в точные проверяемые операции. Подтверждение, кубик, правки, известные маршруты и заметки работают локально.
...
Текущий смысловой компилятор прошёл 178 автоматических тестов и изолированный браузерный прогон; семь зависающих `TestClient`-проверок и живой прогон нового сценового контракта ещё не приняты. Исторические пробы прежнего строгого цикла не считаются приёмкой новой механики. [Сводка и команды](docs/VERIFICATION.md).
