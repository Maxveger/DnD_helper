...
## Следующий этап: свободный мир

**Работает первый текстовый прототип:** откройте **«Свободный мир»** в меню. Codex через аккаунт ChatGPT предлагает последствия свободного действия; код проверяет механику, ведущий принимает исход. Новые места, факты и обещания сохраняются в отдельной базе, прежняя демопартия остаётся на месте.

По умолчанию используется **Luna**; модель можно выбрать на странице игры, независимо от настроек Codex для разработки. Значимые действия пока занимают около 25 секунд из-за нескольких последовательных запросов. [Замеры скорости и расхода](docs/MODEL_COMPARISON.md).

Нужен Codex CLI 0.154.0 или новее. На странице есть проверка подключения и кнопка **«Войти через ChatGPT»**; API-ключ для текстового режима не нужен. Проверены шесть живых ходов, память об обещании, создание места, отказ от задания и отклонение несуществующего предмета. **Это короткий прототип:** Telegram, голос, полный бой, импорт мира и длительная игра ещё впереди. Альтернативный API остаётся задачей нового режима; платного переключения нет.

[Запуск, результаты и ограничения](docs/FREE_WORLD_CHECK.md) · [Полное ТЗ](docs/FREE_WORLD_SPEC.md) · [Подключение Codex](docs/CODEX_PROVIDER.md).
