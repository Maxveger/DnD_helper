...
Деморежим не ограничен таймером или числом действий: приключение можно пройти до финала. Среди вариантов показываются только доступные для выбранного героя и цели действия и переходы **«Перейти: название локации»**. Выполненные одноразовые действия исчезают после принятия исхода; атака и лечение остаются доступны, когда их позволяют правила. После выбора действия подтвердите исход кнопкой «Продолжить», а при проверке сначала введите или бросьте кубик. Кнопка перехода сразу перемещает группу. Без GPT свободный текст распознаётся только по небольшому словарю; неизвестное действие требует выбора ведущего.

Если действие выполнить нельзя, доступная быстрая кнопка заменит его. Для смены локации отмените текущую заявку. Если накопились лишние заявки, кнопка **«Отменить все заявки (N)»** под текущим действием убирает их вместе с текущим уточнением, сохраняя уже принятые события и состояние мира. Во время броска или подтверждения исхода эта кнопка недоступна.
...
Скачайте **DnD-Helper-windows-x64.zip** из [предварительного выпуска v0.1.0-rc.1](https://github.com/Maxveger/DnD_helper/releases/tag/v0.1.0-rc.1), полностью распакуйте и запустите `DnD-Helper.exe`. Папка `_internal` должна оставаться рядом. Python устанавливать не нужно. Сборка проверена на Windows runner; пробный запуск на ноутбуке ведущего с Windows 10/11 ещё предстоит.
...
## Свои сценарии из чата GPT — подготовленные приключения
...
## Подсказчик ведущего с импровизацией

Откройте **«Помощник ведущего»**. Основной экран — крупная реплика для чтения и следующее действие; память находится справа, герои переключаются кнопками. Настройки и свои сценарии открываются отдельно. «Я веду впервые» объясняет порядок игры, «О мире и истории» — фабулу и характер мира.

Вы ведёте игру за столом: вводите заявку выбранного игрока или свой вопрос. **Luna готовит карточку одним запросом**, включая обе ветки броска. Бросок, правки, подтверждение и известные переходы работают без модели. Программа хранит факты, HP и вещи; решение и чтение реплики остаются ведущему.

Доступны **свои сценарии и лор**: комплект для браузерного GPT → документ `dnd-world@1` → локальная проверка → загрузка. Журнал `dnd-world-log@1` сохраняет сценарий и принятые события для продолжения. Пример содержит двух героев и тайну. Прежние сценарии `dnd-adventure@1` работают в прежнем режиме.

Нужен Codex CLI 0.154.0 или новее с собственным входом ведущего через ChatGPT; кнопки входа и проверки есть на странице. API-ключ для текста не нужен, автоматического платного переключения нет. Весь экран предназначен ведущему и содержит секреты; текст для чтения — проверяемый черновик. Telegram и голос к этому режиму пока не подключены.

[Основа проекта](docs/PROJECT_BRIEF.md) · [Что работает и как пользоваться](docs/GM_ASSISTANT.md) · [Измерения и проверки](docs/GM_ASSISTANT_CHECK.md) · [Подключение Codex](docs/CODEX_PROVIDER.md).
...
- [ТЗ следующего этапа: свободный мир, Codex и альтернативный API](docs/FREE_WORLD_SPEC.md)
