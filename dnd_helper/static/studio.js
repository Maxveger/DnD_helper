...
const labels={hero:'Иво',inventory:'При себе',known:'Известно',commitments:'Договорённости',npcs:'NPC и позиции',threats:'Активные угрозы',pending_intents:'Условные намерения'};
...
function renderCapsule(capsule){const root=$('capsule');root.replaceChildren();const scene=node('section','','memory-section');scene.append(node('h3','Ситуация'));scene.append(node('p',capsule.scene));root.append(scene);for(const [key,label] of Object.entries(labels)){const section=node('section','','memory-section');section.append(node('h3',label));const values=capsule[key]||[];if(values.length){const ul=node('ul');for(const value of values)ul.append(node('li',value));section.append(ul);}else section.append(node('p','—','muted'));root.append(section);} }

function capsuleDiff(before,after){const root=node('div','','memory-diff');let changed=false;if(before.scene!==after.scene){changed=true;root.append(node('p','Ситуация: '+after.scene,'added'));}for(const key of Object.keys(labels)){const old=new Set(before[key]||[]),next=new Set(after[key]||[]);for(const value of next)if(!old.has(value)){changed=true;root.append(node('p',`+ ${labels[key]}: ${value}`,'added'));}for(const value of old)if(!next.has(value)){changed=true;root.append(node('p',`− ${labels[key]}: ${value}`,'removed'));}}if(!changed)root.append(node('p','Модель не предлагает менять принятую память.','muted'));return root;}
...
function renderPending(state){const root=$('pending'),pending=state.pending;root.replaceChildren();root.hidden=!pending;if(!pending)return;if(pending.phase==='planning'){root.append(node('div','Живая модель продолжает сцену…','loader'));button(root,'Отменить',()=>command('cancel',{target:'game'}),'quiet');return;}if(pending.phase==='error'){root.append(node('h2','Карточка не получена'));root.append(node('p',pending.error));const controls=node('div','','buttons');button(controls,'Повторить',()=>command('retry',{target:'game'}),'primary');button(controls,'Отклонить',()=>command('reject'),'quiet');root.append(controls);return;}
  ...
  if(pending.phase==='check'){
    const check=card.check,box=node('div','','check-box');box.append(node('h3',`Проверка: ${labelsForStat(check.stat)} · ${labelsForDifficulty(check.difficulty)}`));box.append(node('p',check.reason));const stakes=node('div','','stakes');const success=node('div','','stake');success.append(node('strong','При успехе'));success.append(node('p',check.success_stake));const failure=node('div','','stake');failure.append(node('strong','При неудаче'));failure.append(node('p',check.failure_stake));stakes.append(success,failure);box.append(stakes);const controls=node('div','','buttons');button(controls,'Бросить d20',()=>command('roll'),'primary');const input=node('input');input.type='number';input.min=1;input.max=20;input.placeholder='1–20';input.setAttribute('aria-label','Результат своего d20');controls.append(input);button(controls,'Использовать бросок',()=>{const value=Number(input.value);if(value>=1&&value<=20)command('roll',{value});else showError(Error('Введите результат от 1 до 20.'));});box.append(controls);root.append(box);const generic={introduced_details:[]};root.append(detailsBlock(card,generic));return;
  }
  ...
  const draft=node('div','','draft');const textarea=node('textarea');textarea.id='draft-text';textarea.value=outcome.read_aloud;textarea.maxLength=3000;textarea.setAttribute('aria-label','Редактируемая реплика игрокам');draft.append(textarea);root.append(draft);root.append(node('p',outcome.summary,'muted'));root.append(capsuleDiff(state.capsule,outcome.capsule_after));root.append(detailsBlock(card,outcome));const controls=node('div','','buttons');button(controls,'Принять текст и память',()=>command('accept',{text:$('draft-text').value,apply_capsule:true,capsule:outcome.capsule_after}),'primary');button(controls,'Принять только текст',()=>command('accept',{text:$('draft-text').value,apply_capsule:false}));button(controls,'Отклонить',()=>command('reject'),'quiet');root.append(controls);
}
...
$('save-direction').onclick=()=>command('direct',{text:$('director-note').value});
